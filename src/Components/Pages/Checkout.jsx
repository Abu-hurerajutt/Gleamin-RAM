...
const Checkout = () => {
  ...
  const handleCardNumberChange = (e) => {
    const value = e.target.value.replace(/\D/g, '').slice(0, 16); // Only allow up to 16 digits
  ...
  const handleExpiryDateChange = (e) => {
    let value = e.target.value.replace(/\D/g, '').slice(0, 4); // Only allow up to 4 digits
    if (value.length >= 3) {
  ...
  const handleCvvChange = (e) => {
    const value = e.target.value.replace(/\D/g, '').slice(0, 3); // Only allow up to 3 digits
    setCvv(value);
    setFlipped(!flipped);
...
<div class="flip-card">
    <div className={`flip-card-inner ${flipped ? 'flipped' : ''}`}>
        <div class="flip-card-front">
            ...
            <svg class="cardlogo" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="36" height="36" viewBox="0 0 48 48">
            ...
            <p class="date_8264">{expiryDate}</p>
...
<br />
    <div className="checkout-modal">
      <form className="checkout-form">
        ...
        <div className="divider">or pay using e-wallet</div>
        
        <div className="payment-options">
          <button className="payment-button " ><SiPaypal/></button>
          <button className="payment-button "><FaApple /></button>
          <button className="payment-button "><FaGooglePay /></button>
        </div>
      </form>
